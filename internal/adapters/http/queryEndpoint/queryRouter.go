...
func StartQueryRouter(e *echo.Echo, db *sql.DB, log *zap.SugaredLogger) {
	...
	e.GET("/query/inventory/summary/wallet/:inventory_id", GetInventoryWithWallet(db, log))
	e.GET("/query/inventory/summary/wallet/items/:inventory_id", GetInventoryWithWalletAndItems(db, log))
	e.GET("/query/inventory/summary/items/:inventory_id", GetInventoryWithItems(db, log))
